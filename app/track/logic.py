...
def add_track_to_graph(file_loader):
    def create_key(element, attribute='longitude'):
    ...
    grouped_points = get_all_points_in_graph_by_grouping_of_attribute(all_points, create_key)
    ...
    for point_to_add in reduced_points:
        ...
        if not prev_point:
            new_point = add_point_to_graph(point_to_add, grouped_points, create_key)
        else:
            new_point = add_point_and_relation_to_exist_point_to_graph(
                ...
                grouping_creation_key=create_key)
